...
| <span class="opa-keep-it-together">`value := object.dig(object, path, default)`</span> | `value` is the value stored by the `object` at the nested `path`. If no value is found, `default` is returned. For example: `object.dig({"a": {"b": "c"}}, ["a", "b"], "x")` will result in `"c"`.| SDK-dependent |
