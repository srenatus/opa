...
| <span class="opa-keep-it-together">`str_addr opa_eval(entrypoint_id, value_addr, str_addr, int32, addr)`</span> | One-off policy evaluation method. Its arguments are everything needed to evaluate: entrypoint, address of data in memory, address and length of input JSON string in memory, and heap address to use. Returns the address to the serialised result value. | 1.2 |
