...
char *opa_eval_ctx_one_off(int entrypoint, opa_value *data, uint32_t heap, size_t input_len)
{
    opa_heap_ptr_set(heap);
    opa_eval_ctx_t ctx = { .entrypoint = entrypoint, .data = data };

    char *input = opa_malloc(input_len * sizeof(input));
    if (opa_host_input(input) != 0) {
        opa_abort("read input");
    }
    ctx.input = opa_value_parse(input, input_len);
